...
export type IDBTransactionCallback<T extends CollectionSchema, R extends RelationsSchema> = (ctx: AsyncIDBInstance<T, R>["collections"], tx: IDBTransaction) => unknown;
export type IDBTransactionFunction<T extends CollectionSchema, R extends RelationsSchema> = <CB extends IDBTransactionCallback<T, R>>(callback: CB, options?: TransactionOptions) => Promise<ReturnType<CB>>;
...
export type OnDBUpgradeCallback<T extends CollectionSchema, R extends RelationsSchema> = (ctx: OnDBUpgradeCallbackContext<T, R>, event: IDBVersionChangeEvent) => Promise<void>;
...
export type CollectionEventCallback<T extends AnyCollection, U extends CollectionEvent> = (data: U extends "clear" ? null : CollectionRecord<T>) => void;
...
export type CollectionKeyPathType<T extends AnyCollection, KeyPath = T["keyPath"]> = KeyPath extends keyof T[typeof $COLLECTION_INTERNAL]["record"] ? T[typeof $COLLECTION_INTERNAL]["record"][KeyPath] : never;
export type CollectionIndex<RecordType extends Record<string, any>> = {
    name: string;
    key: RecordKeyPath<RecordType>;
    options?: IDBIndexParameters;
};
export declare enum CollectionIDMode {
    UserAssigned = "userAssigned",
    AutoIncrement = "autoIncrement"
}
export type RecordKeyPath<RecordType extends Record<string, any>> = (keyof RecordType & string) | ((keyof RecordType & string)[] & NonEmptyArray);
type GetTargetCollectionForRelation<R extends RelationsSchema, SourceCollection extends AnyCollection, RelationName extends string> = {
    [K in keyof R]: R[K] extends Relations<infer From, infer To, infer RelMap> ? From extends SourceCollection ? RelationName extends keyof RelMap ? To : never : never : never;
}[keyof R];
type RelationWithOptionsForCollection<R extends RelationsSchema, T extends AnyCollection> = {
    ...
    with?: T extends AnyCollection ? {
        [K in ValidRelationNamesForCollection<R, T> & string]?: boolean | RelationWithOptionsForCollection<R, GetTargetCollectionForRelation<R, T, K>>;
    } : Record<string, boolean | RelationWithOptionsForCollection<R, any>>;
};
export type RelationWithOptions<R extends RelationsSchema> = RelationWithOptionsForCollection<R, any>;
type ValidRelationNamesForCollection<R extends RelationsSchema, CurrentCollection extends AnyCollection> = {
    [K in keyof R]: R[K] extends Relations<infer From, any, infer RelMap> ? From extends CurrentCollection ? keyof RelMap : never : never;
}[keyof R];
...
type FindRelationForName<R extends RelationsSchema, RelationName extends string> = {
    [K in keyof R]: R[K] extends Relations<any, infer To, infer RelMap> ? RelationName extends keyof RelMap ? RelMap[RelationName] extends {
        type: infer Type;
    } ? Type extends "one-to-one" ? CollectionRecord<To> : Type extends "one-to-many" ? CollectionRecord<To>[] : never : never : never : never;
}[keyof R];
type ProcessNestedRelations<R extends RelationsSchema, WithOptions extends Record<string, any>, RelationName extends string> = WithOptions[RelationName] extends {
    with: infer NestedWith;
} ? NestedWith extends Record<string, any> ? FindRelationForName<R, RelationName> extends Array<infer ArrayElement> ? (ArrayElement & Prettify<MapRelationsToTypes<R, NestedWith>>)[] : FindRelationForName<R, RelationName> extends infer SingleElement ? SingleElement extends null | undefined ? (SingleElement & Prettify<MapRelationsToTypes<R, NestedWith>>) | null : SingleElement & Prettify<MapRelationsToTypes<R, NestedWith>> : never : FindRelationForName<R, RelationName> : FindRelationForName<R, RelationName>;
type MapRelationsToTypes<R extends RelationsSchema, WithOptions extends Record<string, any>> = {
    [K in keyof WithOptions & string]: ProcessNestedRelations<R, WithOptions, K> extends Array<infer T> ? T[] : ProcessNestedRelations<R, WithOptions, K> | null;
};
export type RelationResult<T extends AnyCollection, R extends RelationsSchema, Options extends FindOptions<R, T>> = Options extends {
    with: infer With;
} ? With extends Record<string, any> ? CollectionRecord<T> & Prettify<MapRelationsToTypes<R, With>> : CollectionRecord<T> : CollectionRecord<T>;
export type RelationsWith<R extends RelationsSchema, _CollectionName extends string> = Record<string, boolean | RelationWithOptions<R>>;
export type RelationsWithOptions<R extends RelationsSchema, _CollectionName extends string> = RelationWithOptions<R>;
export {};
